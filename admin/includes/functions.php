...
function ihc_check_payment_status($p_type){
	...
	switch($p_type){
		case 'payir':
					  $arr = ihc_return_meta_arr('payment_payir');
					  if ($arr['ihc_payir_status'] == 1) { $return['active'] = 'payir-active'; $return['status'] = 1; }
					  if ($arr['ihc_payir_key'] != '') $return['settings'] = 'Completed';
					  break;
		case 'bank_transfer':
...
function ihc_check_payment_gateways(){
	...
	if ($levels){
		...
		if ($paid_levels){
			$payments_gateways = array('payir','bank_transfer');
			$err_msg = TRUE;
