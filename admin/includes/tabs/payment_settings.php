<div class="ihc-subtab-menu">
	<a class="ihc-subtab-menu-item" href="<?php echo $url.'&tab='.$tab.'&subtab=payir';?>"><?php _e('Payir', 'ihc');?></a>
...
if (empty($_GET['subtab'])){
	...
	<div class="iump-payment-list-wrapper">
		...
		<?php $pay_stat = ihc_check_payment_status('payir'); ?>
		  <a href="<?php echo $url.'&tab='.$tab.'&subtab=payir';?>">
			<div class="iump-payment-box <?php echo $pay_stat['active']; ?>">
				<div class="iump-payment-box-title">درگاه Pay.ir</div>
	...
	switch ($_GET['subtab']){
		case 'payir':
			ihc_save_update_metas('payment_payir');//save update metas
			$meta_arr = ihc_return_meta_arr('payment_payir');//getting metas
			$pages = ihc_get_all_pages();//getting pages
			echo ihc_check_default_pages_set();//set default pages message
			echo ihc_check_payment_gateways();
			?>
			...
			<form action="" method="post">
					<div class="ihc-stuffbox">
						<h3><?php echo "فعالسازی درگاه پرداخت Pay.ir";?></h3>
						<div class="inside">		
							<div class="iump-form-line">
								<h4><?php echo "بعد از اتمام تنظیمات، فعال سازی درگاه پرداخت Pay.ir را می توانید در اینجا انجام دهید ";?> </h4>
								<label class="iump_label_shiwtch" style="margin:10px 0 10px -10px;">
								<?php $checked = ($meta_arr['ihc_payir_status']) ? 'checked' : '';?>
								<input type="checkbox" class="iump-switch" onClick="iump_check_and_h(this, '#ihc_payir_status');" <?php echo $checked;?> />
								<div class="switch" style="display:inline-block;"></div>
							</label>
							<input type="hidden" value="<?php echo $meta_arr['ihc_payir_status'];?>" name="ihc_payir_status" id="ihc_payir_status" />
							</div>
					...
					<div class="ihc-stuffbox">
						...
						<h3><?php echo "پیکربندی Pay.ir";?></h3>
						
						<div class="inside">
							<div class="iump-form-line">
								<label class="iump-labels"><?php _e('Api Key:', 'ihc');?></label> <input type="text" value="<?php echo $meta_arr['ihc_payir_key'];?>" name="ihc_payir_key" style="width: 300px;" />
							</div>

							<div class="iump-form-line iump-special-line">
								<label class="iump-labels-special"><?php _e('Redirect Page after Payment:', 'ihc');?></label>
								<select name="ihc_payir_return_page">
									<option value="-1" <?php if($meta_arr['ihc_payir_return_page']==-1)echo 'selected';?> >...</option>
									<?php 
										if($pages){
											foreach($pages as $k=>$v){
												?>
													<option value="<?php echo $k;?>" <?php if ($meta_arr['ihc_payir_return_page']==$k) echo 'selected';?> ><?php echo $v;?></option>
												<?php 
											}						
										}
									?>
					...
					<div class="ihc-stuffbox">
						...
						<div class="inside">
							<div class="iump-form-line iump-no-border">
								...
								<input type="text" name="ihc_payir_label" value="<?php echo $meta_arr['ihc_payir_label'];?>" />
							</div>
							
							<div class="iump-form-line iump-no-border">
								<label class="iump-labels"><?php _e('Order:', 'ihc');?></label>
								<input type="number" min="1" name="ihc_payir_select_order" value="<?php echo $meta_arr['ihc_payir_select_order'];?>" />
							</div>						
