	...
	switch ($type){
		case 'payment':
			$arr = array(
							...
							'ihc_payment_selected' => 'payir',
						);
		break;
		case 'payment_payir':
			$arr = array(
							'ihc_payir_key' => '',
							'ihc_payir_return_page' => -1,
							'ihc_payir_status' => 0,
							'ihc_payir_label' => 'Payir',
							'ihc_payir_select_order' => 1,
						);
		break;
...
function ihc_cancel_level($u_id, $l_id){
	...
	if ($txn_id && $payment_type){
		...
		switch ($payment_type){
			case 'payir':
				$alias = get_option('ihc_payir_key');
...
function ihc_check_payment_available($type=''){
	...
	if ($type){
		...
		switch ($type){
			case 'payir':
				if (!empty($payment_metas['ihc_payir_key']) && !empty($payment_metas['ihc_payir_status'])){
					return TRUE;
				}
				break;
			case 'authorize':
